...
# --- 1. SAYFA AYARLARI ---
st.set_page_config(
    page_title="SiberKalkan v2.0",
    page_icon="🛡️",
    layout="wide",
    initial_sidebar_state="expanded"
)

# --- 2. CSS TASARIMI (ANACONDA İLE BİREBİR AYNI) ---
st.markdown("""
<style>
...
def model_yukle():
    try:
        ...
        # 1. YEREL MODEL KONTROLÜ (Anaconda İçin - Aynen Korundu)
        if os.path.exists(model_path):
            tokenizer = BertTokenizer.from_pretrained(model_path, local_files_only=True)
            model = BertForSequenceClassification.from_pretrained(model_path, local_files_only=True)
            model_type = "local"
        else:
            # 2. BULUT MODEL DÜZELTMESİ (GitHub İçin)
            # Rastgele "dbmdz" yerine AKILLI "savasy" modeli kullanıldı
    ...
    with col_result:
        if analyze_btn and user_input and model:
            ...
            if kural_ihlali:
                ...
                # --- BULUT / YEREL FARKINI BURADA DÜZELTTİK ---
                if model_type == "cloud":
                    # Savasy Modeli: Index 0 = Negatif, Index 1 = Pozitif
                    # Negatif skoru alıyoruz (Index 0)
                    score_neg = probs[0][0].item()
                else:
                    # Senin Eğittiğin Model: (Anaconda ayarın)
                    score_neg = probs[0][0].item()
        ...
        if not st.session_state.get('alert_active'):
                ...
                with st.form("chat_form_input", clear_on_submit=True):
                    ...
                    if submitted and user_msg:
                        ...
                        if kural: 
                            ...
                            # --- MOBİL İÇİN SKOR DÜZELTMESİ ---
                            if model_type == "cloud":
                                # Cloud (Savasy): Index 0 = Negatif
                                neg_score = probs[0][0].item()
                            else:
                                # Yerel: Index 0 = Zorbalık
                                neg_score = probs[0][0].item()
            ...
            with st.form("chat_form_alert"):
                if is_game_mode:
                    ...
                    c1, c2 = st.columns(2)
                    with c1:
                        # --- VAZGEÇ TUŞU (RAPORA KAYIT EKLENDİ) ---
                    ...
                    # --- EĞİTİM MODU (RAPORA KAYIT EKLENDİ) ---
